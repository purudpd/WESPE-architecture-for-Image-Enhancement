    ...
    if phone.endswith("_orig"):
        ...
        for i in iteration:
            ...
            for photo in test_photos:
                ...
                src_img = misc.imread(test_dir + photo)
                print('SHAPE:',src_img.shape)
                image = np.float16(misc.imresize(src_img, res_sizes[phone])) / 255

                image_crop = utils.extract_crop(image, resolution, phone, res_sizes)
                image_crop_2d = np.reshape(image_crop, [1, IMAGE_SIZE])

                print('Image_Crop_2d:',image_crop_2d.shape)
